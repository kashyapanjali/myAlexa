...
def ask():
    ...
    if "hi" in question or "hello" in question:
        ...
        return jsonify({'answer': "Good morning! Have a productive day!"})
    elif "good night" in question:
        return jsonify({'answer': "Good night! Sleep well 😴"})
    elif "how are you" in question:
        return jsonify({'answer': "I'm just code, but I'm feeling great! How about you?"})
    elif "what's the time" in question or "current time" in question:
        current_time = datetime.datetime.now().strftime("%I:%M %p")
        return jsonify({'answer': f"The current time is {current_time}."})
    elif "date today" in question or "what's the date" in question:
